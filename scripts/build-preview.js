...
function genPage(icons) {
  ...
  for (const [cat, items] of Object.entries(cats)) {
    navBtns += `<button data-c="${cat}">${labels[cat] || cat} (${items.length})</button>`;
    catData[cat] = items.map(i => ({
      id: i.id, name: i.name, cat: i.cat,
      url: i.url, isSvg: i.isSvg
    }));
  }
  const jsonStr = JSON.stringify(catData).replace(/</g, '\\u003c');

  // Build cards as plain DOM strings - NO nested quotes
  let cardFn = '';
  cardFn += 'function card(i){';
  cardFn += 'var ic;';
  cardFn += 'if(i.isSvg){ic=\'<svg viewBox=\\'0 0 24 24\\'><use href=\\'' + ' + 'i.url' + '\\'></use></svg>\';}';
  cardFn += 'else{ic=\'<img src=\\'' + ' + 'i.url' + '\\' alt=\\'\\' loading=lazy onerror=this.parentNode.innerHTML=\\'<span>fail</span>\\'>\';}';
  cardFn += 'return \'<div class=c data-id=\\'' + ' + 'i.id' + '\\'><div class=ic>\'+ic+\'</div><span class=n>\'+i.name+\'</span><span class=i>\'+i.id+\'</span></div>\';';
  cardFn += '}';
...
<div class="nv" id="nv"><button data-c="all" class="on">All (${icons.length})</button>${navBtns}</div>
    ...
    if (!query && cat !== 'all') {
      ...
      if (pages > 1) {
        ...
        for (var p = 0; p < pages; p++) {
          bt += '<button class="' + (p === pg ? 'on' : '') + '" onclick="go(' + p + ')">' + (p + 1) + '</button>';
        }
        pagination = '<div class="pg">' + bt + '</div>';
      }
    } else if (cat === 'all' && !query) {
      displayItems = list.slice(0, PER);
      if (list.length > PER) {
        pagination = '<div class="pg"><button class="on">1</button><span style="color:#666;font-size:.75rem;margin-left:.3rem">+' + (list.length - PER) + ' more</span></div>';
      }
    }
    
    var cards = '';
    for (var k = 0; k < displayItems.length; k++) {
      cards += card(displayItems[k]);
    }
